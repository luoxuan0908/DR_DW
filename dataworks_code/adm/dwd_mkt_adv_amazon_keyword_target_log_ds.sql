...
--********************************************************************--
--author:Ada
--create time:2024-05-03 21:53:45
--********************************************************************--

CREATE TABLE IF NOT EXISTS whde.dwd_mkt_adv_amazon_keyword_target_log_ds(
    ...
    STORED AS ALIORC
    TBLPROPERTIES ('comment'='广告关键词投放操作日志')
    LIFECYCLE 2000;

INSERT OVERWRITE TABLE dwd_mkt_adv_amazon_keyword_target_log_ds PARTITION (ds = '${bizdate}')
SELECT  tenant_id,
    profile_id
     ,'SP' ad_type
     ,keyword_id
...
--        ,"fenghuo_common_keyword" table_src
--        ,'${bizdate}' data_dt
--        ,GETDATE()  etl_data_dt
--        ,ds
--        FROM
...
--        ,SUBSTR(hs,1,8) ds
--FROM    open_ods.ods_fenghuo_fenghuo_metadata_2_fenghuo_common_keyword
