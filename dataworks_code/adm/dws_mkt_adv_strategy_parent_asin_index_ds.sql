--@exclude_input=whde.ods_asq_jdy_asin_list
--odps sql
--********************************************************************--
--author:Ada
--create time:2024-04-28 21:18:57
--********************************************************************--

CREATE TABLE IF NOT EXISTS dws_mkt_adv_strategy_parent_asin_index_ds
(
...
from dws_mkt_adv_strategy_parent_asin_index_tmp02_1 q1
    ...
    select tenant_id
         ...
         ,marketplace_id
         ,seller_id
    from  whde.dwd_sit_shp_amazon_seller_sites_store_df            --授权店铺清单
    where ds = max_pt('whde.dwd_sit_shp_amazon_seller_sites_store_df ') and profile_id is not null
    group by tenant_id
           ,profile_id
           ,marketplace_id
           ,seller_id
    ...
    select concat(case when country = '阿联酋' then '阿拉伯联合酋长国' else country end,'站') as marketplace_name
         ...
         ,case when max(season) = '四季' then '季中'
               when max(season) = '春夏季' and month(to_date('${bizdate}','yyyymmdd')) = 2 then '季前'
                       when max(season) = '春夏季' and month(to_date('${bizdate}','yyyymmdd')) between 3 and 7 then '季中'
                       when max(season) = '春夏季' and month(to_date('${bizdate}','yyyymmdd')) > 7 then '过季'
                       when max(season) = '秋冬季' and month(to_date('${bizdate}','yyyymmdd')) = 8 then '季前'
                       when max(season) = '秋冬季' and month(to_date('${bizdate}','yyyymmdd')) between 9 and 12 then '季中'
                       when max(season) = '秋冬季' and month(to_date('${bizdate}','yyyymmdd')) < 8 then '过季' end as season_label
                 ,case when max(season) = '秋冬季' then '10001' when max(season) = '春夏季' then '94513' when max(season) = '四季' then '33166' end as postcode
           from whde.ods_asq_jdy_asin_list      --自带品类
...
select s1.tenant_id
     ,s1.marketplace_id
     ,s1.parent_asin
     ,to_char(s1.create_date,'yyyymmdd') as adv_start_date
     ,datediff(to_date('${bizdate}','yyyymmdd'),s1.create_date,'dd') as adv_days
     ,round(datediff(to_date('${bizdate}','yyyymmdd'),s1.create_date,'dd') / 7,1) as adv_weeks
from (
         select q1.tenant_id
              ,q3.marketplace_id
              ,q3.parent_asin
...
--支持重跑
alter table dws_mkt_adv_strategy_parent_asin_index_ds drop if exists partition (ds = '${bizdate}');

--插入最新数据
insert into table dws_mkt_adv_strategy_parent_asin_index_ds partition (ds = '${bizdate}')
(
 tenant_id
,profile_id
,marketplace_id
...
from dws_mkt_adv_strategy_parent_asin_index_tmp02_2 q1
         inner join whde.dim_user_permission_info_df  q2  --dws_mkt_adv_asq_parent_asin_manager_ds q2
