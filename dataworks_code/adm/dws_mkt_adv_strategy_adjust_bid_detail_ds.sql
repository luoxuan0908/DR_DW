...
--********************************************************************--
--author:Ada
--create time:2024-05-30 00:36:33
--********************************************************************--
CREATE TABLE IF NOT EXISTS dws_mkt_adv_strategy_adjust_bid_detail_ds
(
    ...
    ,bid_adjust_rate decimal(18,6) comment '竞价调整幅度'
    , bid decimal(18,6) comment '当前竞价'
...
(
    ...
    STORED AS ALIORC
    TBLPROPERTIES ('comment' = '广告策略调整竞价(子表)')
    LIFECYCLE 366
;
...
--基础数据
drop table if exists dws_mkt_adv_strategy_adjust_bid_detail_ds_tmp01;
create table dws_mkt_adv_strategy_adjust_bid_detail_ds_tmp01 as
select   tenant_id
     ,profile_id
     ,marketplace_id
     ,marketplace_name
...
where ds = '${nowdate}'  --T日
  and adv_days >= 14 and nvl(parent_asin,'') <> '' and ad_mode = '手动投放'
;


--精准匹配的投放词
drop table if exists dws_mkt_adv_strategy_adjust_bid_detail_ds_tmp01_1;
create table dws_mkt_adv_strategy_adjust_bid_detail_ds_tmp01_1 as
select tenant_id
     ,profile_id
     ,campaign_id
     ,ad_group_id
     ,keyword_id
     ,keyword_text
     ,cast(bid as decimal(18,6)) as bid
     ,cast(0.1 as decimal(18,6)) as bid_adjust_rate
from whde.adm_amazon_adv_keyword_target_status_df
where ds = max_pt('whde.adm_amazon_adv_keyword_target_status_df') and match_type = 'EXACT' and status = 'ENABLED'
;


--成熟期、库存充足、有类目平均筛选
drop table if exists  dws_mkt_adv_strategy_adjust_bid_detail_ds_tmp02;
create table dws_mkt_adv_strategy_adjust_bid_detail_ds_tmp02 as
select  tenant_id
     ,profile_id
     ,marketplace_id
     ,seller_id
...
where   ds = max_pt('whde.dws_mkt_adv_strategy_parent_asin_base_index_new_ds')
  and life_cycle_label = '成熟期' and stock_label = '库存充足'
;

-------------算库存标签>>结束--------------

--大小词标签
drop table if exists dws_mkt_adv_strategy_adjust_bid_detail_ds_tmp05;
create table dws_mkt_adv_strategy_adjust_bid_detail_ds_tmp05 as
...
--报错 table whde.dws_mkt_adv_strategy_search_term_pasin_rank_new_ds cannot be resolved

--搜索词排名
drop table if exists dws_mkt_adv_strategy_adjust_bid_detail_ds_tmp06;
create table dws_mkt_adv_strategy_adjust_bid_detail_ds_tmp06 as
select  top_parent_asin as parent_asin
     ...
     ,marketplace_id
     ,post_code
from whde.dws_mkt_adv_strategy_search_term_pasin_rank_new_ds
where ds = max_pt('whde.dws_mkt_adv_strategy_search_term_pasin_rank_new_ds') and term_type = '搜索词'
;


--搜索词爬虫清单
drop table if exists dws_mkt_adv_strategy_adjust_bid_detail_ds_tmp06_1;
create table dws_mkt_adv_strategy_adjust_bid_detail_ds_tmp06_1 as
select marketplace_id
     ,search_term
     ,post_code
from dws_mkt_adv_strategy_adjust_bid_detail_ds_tmp06
...
--正在守坑的词
drop table if exists dws_mkt_adv_strategy_adjust_bid_detail_his;
create table dws_mkt_adv_strategy_adjust_bid_detail_his as
select   tenant_id
     ,profile_id
     ,campaign_id
     ,ad_group_id
...
from    (
            select tenant_id
                 ...
                 ,top_parent_asin
                 ,target_term
            from whde.dws_mkt_adv_strategy_adjust_bid_param_hf --每小时跑
            where hs = (select max(hs) from whde.dws_mkt_adv_strategy_adjust_bid_param_hf where hs is not null) and adv_manager_id <> '1'
            ...
            select   tenant_id
                 ...
                 ,top_parent_asin
                 ,target_term
            from whde.dws_mkt_adv_strategy_opt_adjust_bid_param_ds  --寻优
            where ds = (select max(ds) from whde.dws_mkt_adv_strategy_opt_adjust_bid_param_ds where ds is not null) and adv_manager_id <> '1'
            ...
            select  tenant_id
                 ...
                 ,top_parent_asin
                 ,target_term
            from whde.dws_mkt_adv_strategy_search_term_adjust_bid_param_hf
            where hs = (select max(hs) from whde.dws_mkt_adv_strategy_search_term_adjust_bid_param_hf where hs is not null) and adv_manager_id <> '1'
        )
group by  tenant_id
       ,profile_id
       ,campaign_id
...
-- --搜索词的垄断和中国卖家占比指标
-- drop table if exists dws_mkt_adv_strategy_adjust_bid_detail_ds_tmp07;
-- create table dws_mkt_adv_strategy_adjust_bid_detail_ds_tmp07 as
-- select marketplace_id
--       ,keyword as search_term
--       ,cast(sale_monopoly_rate as decimal(18,6)) as sale_monopoly_rate
--       ,cast(chn_seller_rate as decimal(18,6)) as chn_seller_rate
...
drop table if exists dws_mkt_adv_strategy_adjust_bid_detail_asq_final;
create table dws_mkt_adv_strategy_adjust_bid_detail_asq_final as
select tenant_id
     ,profile_id
     ,marketplace_id
     ,marketplace_name
     ...
     ,case when p1_norm_label = 1 then '自然排名第一页' end as norm_rank_label
     ,'ALL' as adv_rank_label
     ,case when clicks >= (1/cate_cvr) * 1.2 then '有效点击' end as click_label
     ,case when cvr >= cate_cvr then '高转化' end as cvr_label
     ,bid_adjust_rate
     ,bid
from (
         select   q1.tenant_id
              ...
              ,q6.bid_adjust_rate
              ,q6.bid
         from dws_mkt_adv_strategy_adjust_bid_detail_ds_tmp01 q1
                  inner join dws_mkt_adv_strategy_adjust_bid_detail_ds_tmp02 q2  --基础筛选
                  ...
                  inner join dws_mkt_adv_strategy_adjust_bid_detail_ds_tmp01_1 q6  --精准匹配的投放词
                  ...
                  left join dws_mkt_adv_strategy_adjust_bid_detail_ds_tmp05 q3 --大小词
                            on q1.marketplace_id = q3.marketplace_id and tolower(q1.target_text) = tolower(q3.search_term)
                  left join dws_mkt_adv_strategy_adjust_bid_detail_ds_tmp06 q4 --投放词排名
                  ...
                  left join dws_mkt_adv_strategy_adjust_bid_detail_ds_tmp06_1 q7  --搜索词爬虫清单
                            on q1.marketplace_id = q7.marketplace_id and tolower(q1.target_text) = tolower(q7.search_term) and q2.post_code = q7.post_code
                  left join dws_mkt_adv_strategy_adjust_bid_detail_his q5 --剔除已经在守坑的
         ...
         where   q5.target_term is null and q2.cate_cvr <> 0
     )s1
;


--支持重跑
alter table dws_mkt_adv_strategy_adjust_bid_detail_ds drop if exists partition (ds = '${nowdate}');


--插入最新数据
insert into table dws_mkt_adv_strategy_adjust_bid_detail_ds partition (ds = '${nowdate}')
(
    tenant_id
   ,row_id
   ,strategy_id
...
from dws_mkt_adv_strategy_adjust_bid_detail_zt_final q1
         ...
         left join dws_mkt_adv_strategy_adjust_bid_detail_ds_tmp05 q3
                   on q1.marketplace_id = q3.marketplace_id and tolower(q1.target_term) = tolower(q3.search_term)
where q2.action_name = '投放词守坑' and q2.life_cycle_label = '成熟期' and q1.term_type_label = '投放词'
  and tolower(q1.target_term) not like '%lefflow%'
  and tolower(q1.target_term) not like '%joinfree%'
  and tolower(q1.target_term) not like '%besroad%'
  and tolower(q1.target_term) not like '%lisdwde%'
  and tolower(q1.target_term) not like '%skaso%'
  and tolower(q1.target_term) not like '%qfh%'
;
