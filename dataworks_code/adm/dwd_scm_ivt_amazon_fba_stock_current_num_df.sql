...
--********************************************************************--
--author:Ada
--create time:2024-05-01 16:15:37
--********************************************************************--
CREATE TABLE IF NOT EXISTS whde.dwd_scm_ivt_amazon_fba_stock_current_num_df(
...
INSERT OVERWRITE TABLE  dwd_scm_ivt_amazon_fba_stock_current_num_df PARTITION (ds = '${bizdate}')
SELECT
    tenant_id,
    report_id,
    record_id,
...
FROM
    (SELECT  tenant_id,
     ...
     FROM   (select
             ...
             from whde.get_fba_myi_unsuppressed_inventory_data
             where  pt = '${bizdate}'
