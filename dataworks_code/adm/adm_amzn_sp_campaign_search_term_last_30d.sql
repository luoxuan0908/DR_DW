...
--********************************************************************--
--author:Ada
--create time:2024-03-03 19:22:09
--********************************************************************--


CREATE TABLE IF NOT EXISTS whde.adm_amzn_sp_campaign_search_term_last_30d(
...
--广告活动下的花费最高的父asin
INSERT OVERWRITE TABLE adm_amzn_sp_campaign_search_term_last_30d PARTITION (ds = '${bizdate}')
--最近30天的搜索词的效果
SELECT  a.tenant_id
     ,a.profile_id
     ,a.marketplace_id
...
FROM    (
            SELECT  s1.tenant_id
                 ...
                 ,SUM(clicks) clicks
                 ,SUM(cost) cost
                 ,SUM(sales_7d) sale_amt
                 ,SUM(units_sold_clicks_7d) order_num
            ...
            FROM    (
                ...
                FROM    whde.adm_amzn_advertised_asin_info_last_30d  --adm_amazon_adv_sku_wide_d
                WHERE   ds = '${bizdate}'
                GROUP BY tenant_id
                       ,profile_id
                       ,marketplace_id
    ...
    SELECT  tenant_id
         ...
         ,fba_instock_cnt
    FROM    open_dw.dws_itm_spu_amazon_parent_asin_index_df
    WHERE   ds = MAX_PT('open_dw.dws_itm_spu_amazon_parent_asin_index_df')
) b --父aisn、以及类目、库存
    ...
    SELECT  tenant_id
         ...
         ,cate_cpc
         ,cate_acos
    FROM    open_dw.adm_amazon_adv_strategy_category_d
    WHERE   ds = '${bizdate}'
) d --类目数据
    ...
    FROM    open_dw.dwd_itm_spu_amazon_search_keyword_info_ws
    WHERE   ws = MAX_PT('open_dw.dwd_itm_spu_amazon_search_keyword_info_ws')
    GROUP BY marketplace_id
           ,search_term
) e --aba搜索排名
                        ON      TOLOWER(a.search_term) = TOLOWER(e.search_term)
                            AND a.marketplace_id=e.marketplace_id
            LEFT JOIN   (
    SELECT  marketplace_id
         ,parent_asin
         ,MAX(link) main_asin_url
    FROM    open_dw.dwd_itm_sku_amazon_asin_info_ds
    WHERE   ds = MAX_PT('open_dw.dwd_itm_sku_amazon_asin_info_ds')
      AND     NVL(parent_asin,'') <> ''
    GROUP BY marketplace_id
           ,parent_asin
) f
