...
--********************************************************************--
--author:Ada
--create time:2024-04-27 23:20:57
--********************************************************************--
CREATE TABLE IF NOT EXISTS whde.dws_itm_spu_amazon_parent_asin_index_df(
...
drop table  IF EXISTS  dws_itm_spu_amazon_parent_asin_index_${bizdate}_zt_tmp1;
CREATE  table  IF NOT EXISTS  dws_itm_spu_amazon_parent_asin_index_${bizdate}_zt_tmp1
AS
...
select
    tenant_id	--租户ID
     ...
     ,SUM(afn_reserved_customerorders_num)afn_reserved_customerorders_num	--FBA预留_为买家订单预留的商品数量
     ,SUM(afn_reserved_fc_transfers_num)afn_reserved_fc_transfers_num	--FBA预留_正在从一个运营中心转运至另一运营中心的商品数量
     ,SUM(afn_reserved_fc_processing_num	)afn_reserved_fc_processing_num--FBA预留_搁置在运营中心等待进行其他处理的商品数量，包括与移除订单关联的商品
...
drop table  IF EXISTS  dws_itm_spu_amazon_parent_asin_index_${bizdate}_zt_tmp2;
CREATE  table  IF NOT EXISTS  dws_itm_spu_amazon_parent_asin_index_${bizdate}_zt_tmp2
AS
SELECT
    tenant_id	--租户ID
     ,seller_id	--卖家ID
...
--//取 父asin的信息（主卖asin的信息）


drop table  IF EXISTS  dws_itm_spu_amazon_parent_asin_index_${bizdate}_zt_tmp3;
CREATE  table  IF NOT EXISTS  dws_itm_spu_amazon_parent_asin_index_${bizdate}_zt_tmp3
AS
...
FROM dws_itm_spu_amazon_parent_asin_index_${bizdate}_zt_tmp2 T1
         LEFT JOIN
     (select
          tenant_id	--租户ID
           ,seller_id	--卖家ID
     ...
     ON
         T1.tenant_id=T2.tenant_id	--租户ID
             AND T1.seller_id=T2.seller_id	--卖家ID
             AND T1.marketplace_id=T2.marketplace_id
             AND T1.main_asin=T2.asin
;
...
drop table  IF EXISTS  dws_itm_spu_amazon_parent_asin_index_${bizdate}_zt_tmp4;
CREATE  table  IF NOT EXISTS  dws_itm_spu_amazon_parent_asin_index_${bizdate}_zt_tmp4
AS
SELECT
    T1.tenant_id
     ,T1.marketplace_id
...
drop table  IF EXISTS  dws_itm_spu_amazon_parent_asin_index_${bizdate}_zt_tmp10;
CREATE  table  IF NOT EXISTS dws_itm_spu_amazon_parent_asin_index_${bizdate}_zt_tmp10
AS
select
    T1.tenant_id	--租户ID
     ,T1.seller_id	--卖家ID
...
FROM  dws_itm_spu_amazon_parent_asin_index_${bizdate}_zt_tmp1 T1
          LEFT JOIN  dws_itm_spu_amazon_parent_asin_index_${bizdate}_zt_tmp2 T2
                     ON T1.tenant_id=T2.tenant_id
                         AND T1.seller_id=T2.seller_id
                         AND T1.marketplace_id=T2.marketplace_id
                         AND T1.parent_asin=T2.parent_asin
          LEFT JOIN  dws_itm_spu_amazon_parent_asin_index_${bizdate}_zt_tmp3 T3
          ...
          LEFT JOIN  dws_itm_spu_amazon_parent_asin_index_${bizdate}_zt_tmp4 T4
...
INSERT OVERWRITE TABLE  dws_itm_spu_amazon_parent_asin_index_df  PARTITION (ds = '${bizdate}')
SELECT
    tenant_id,
    seller_id,
    marketplace_id,
    ...
    '${bizdate}' data_dt,
    GETDATE()  etl_data_dt
FROM dws_itm_spu_amazon_parent_asin_index_${bizdate}_zt_tmp10
;
