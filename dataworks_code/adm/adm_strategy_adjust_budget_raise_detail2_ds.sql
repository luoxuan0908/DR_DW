...
--********************************************************************--
--author:Ada
--create time:2024-05-10 21:49:14
--********************************************************************--

drop table if EXISTS adm_strategy_adjust_budget_raise_detail2_ds;
CREATE TABLE IF NOT EXISTS adm_strategy_adjust_budget_raise_detail2_ds
(
    row_id string
    ...
    ,avg_campaign_budget_amt_7d string comment 	'近5天平均每天花费'
    ,campaign_budget_no_per string comment 	'预算不足比例'
...
insert OVERWRITE table adm_strategy_adjust_budget_raise_detail2_ds PARTITION (ds = '${nowdate}')
select distinct
    a.row_id
              ,a.tenant_id
              ,d.tenant_name
