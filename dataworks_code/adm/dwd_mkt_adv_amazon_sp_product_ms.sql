...
drop table if EXISTS dwd_mkt_adv_amazon_sp_product_ms;
CREATE TABLE IF NOT EXISTS whde.dwd_mkt_adv_amazon_sp_product_ms(
...
insert overwrite table whde.dwd_mkt_adv_amazon_sp_product_ms partition (ms)
select
    profile_id
     ,report_id
     ,report_type
...
from (
         ...
         from whde.dwd_amzn_sp_advertised_product_by_advertiser_report_ds  t1
                  inner join (
             select  profile_id
                  ,marketplace_id
                  ,timezone
         ...
         where substring(t1.ds,1,8)>='${bizdate}'
           and substring(t1.ds,1,8)<='${bizdate1}'
