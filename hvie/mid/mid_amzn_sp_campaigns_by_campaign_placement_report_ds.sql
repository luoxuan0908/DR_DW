-- drop table if EXISTS amz.mid_amzn_sp_campaigns_by_campaign_placement_report_ds;
CREATE TABLE IF NOT EXISTS amz.mid_amzn_sp_campaigns_by_campaign_placement_report_ds(
...
with dwd_adv_sp_campaigns_by_campaign_placement_report_df_temp as (
    SELECT
     ...
     COALESCE(campaign_bidding_strategy, '') AS campaign_bidding_strategy,
     COALESCE(placement_classification, '') AS placement_classification,
    ...
    'amzn_sp_campaigns_by_campaign_placement_report' AS table_src,
    '${last_day}' AS data_dt,
     ds
FROM
        (
            SELECT  case when profile_id = 'null' then null else profile_id end as profile_id,
                    ...
                    ,ROW_NUMBER() OVER (PARTITION BY tenant_id,profile_id,campaign_id,placement_classification,report_date ORDER BY ds DESC ) AS rn
                    ,ds
    FROM    ods.ods_report_amzn_sp_campaigns_by_campaign_placement_report_df
            WHERE   ds = '${last_day}'
        ) s
    WHERE rn=1
), n_lag_table AS (
    SELECT
        CASE
            ...
            ELSE ${n_lag2}
            END AS n_lag
    FROM dwd_adv_sp_campaigns_by_campaign_placement_report_df_temp
    WHERE ds = '${last_day}'
)

-- Step 2: 使用计算的 n_lag 插入数据到目标表
INSERT OVERWRITE TABLE amz.mid_amzn_sp_campaigns_by_campaign_placement_report_ds PARTITION (ds)
SELECT
    tenant_id, profile_id, report_id, report_type, seller_id, report_date, data_last_update_time, country_code, campaign_id,
    campaign_name, campaign_status, campaign_budget_amt, campaign_budget_type, campaign_budget_currency_code, campaign_bidding_strategy,
    placement_classification, impressions, clicks, cost_per_click, click_through_rate, cost, purchases_1d, purchases_7d, purchases_14d,
    purchases_30d, purchases_same_sku_1d, purchases_same_sku_7d, purchases_same_sku_14d, purchases_same_sku_30d, units_sold_clicks_1d,
    units_sold_clicks_7d, units_sold_clicks_14d, units_sold_clicks_30d, sales_1d, sales_7d, sales_14d, sales_30d, attributed_sales_same_sku_1d,
    attributed_sales_same_sku_7d, attributed_sales_same_sku_14d, attributed_sales_same_sku_30d, units_sold_same_sku_1d, units_sold_same_sku_7d,

    units_sold_same_sku_14d, units_sold_same_sku_30d, kindle_edition_normalized_pages_read_14d, kindle_edition_normalized_pages_royalties_14d,data_src,table_src,
    current_date() AS etl_data_dt, date_format(report_date, 'yyyyMMdd') AS ds
FROM (
         SELECT
             tenant_id, profile_id, report_id, report_type, seller_id, report_date, data_last_update_time, country_code, campaign_id,
             campaign_name, campaign_status, campaign_budget_amt, campaign_budget_type, campaign_budget_currency_code, campaign_bidding_strategy,
             ...
             ROW_NUMBER() OVER (PARTITION BY tenant_id, profile_id, campaign_id, placement_classification, report_date ORDER BY current_date() DESC) AS rn
         FROM dwd_adv_sp_campaigns_by_campaign_placement_report_df_temp
