...
CREATE TABLE IF NOT EXISTS amz.mid_amzn_all_orders_df(
    ...
    data_last_update_time timestamp COMMENT '报告数据亚马逊生产时间（已转换成北京时间）',
    ...
    purchase_time timestamp COMMENT '订单创建时间（已转换成北京时间）',
    ori_last_update_time STRING COMMENT '订单最近更新时间',
    last_update_time timestamp COMMENT '订单最近更新时间（已转换成北京时间）',
    ...
    etl_data_dt date COMMENT '数据加载时间'
    )
...
with temp1_dwd_trd_ord_amazon_flat_file_all_orders_report_dk as (
select
    report_type,
    seller_id,
    marketplace_id,
...
from  (SELECT id,
       ...
       from ods.ods_report_get_flat_file_all_orders_data_by_order_date_general_df -- whde.get_flat_file_all_orders_data_by_order_date_general
       where ds ='${last_day}'
      ) t
),temp2_dwd_trd_ord_amazon_flat_file_all_orders_report_dk as (
select report_type,
       seller_id,
       marketplace_id,
       report_id,
       ...
       from_unixtime(unix_timestamp(
                             concat_ws(' ', purchase_date_part, purchase_time_part),
                             'yyyy-MM-dd HH:mm:ss') +
                     8 * 3600)                               AS purchase_time,
       ori_last_update_time,
       from_unixtime(unix_timestamp(
                             concat_ws(' ', last_update_date_part, last_update_time_part),
                             'yyyy-MM-dd HH:mm:ss') + 8 *
                                                      3600)  AS last_update_time,
       order_status,
       fulfillment_channel,
       sales_channel,
       order_channel,
...
from temp1_dwd_trd_ord_amazon_flat_file_all_orders_report_dk
),temp3_dwd_trd_ord_amazon_flat_file_all_orders_report_dk as (
select report_type,
        seller_id,
        marketplace_id,
        report_id,
   ...
   select report_type,
           ...
           merchant_order_id,
           ori_purchase_time,
           cast(purchase_time as timestamp) as purchase_time,
           ori_last_update_time,
           ...
           data_dt,
           etl_data_dt
   from temp2_dwd_trd_ord_amazon_flat_file_all_orders_report_dk
   union all
   select report_type,
          seller_id,
          marketplace_id,
...
----插入
INSERT OVERWRITE TABLE amz.mid_amzn_all_orders_df partition(ds = '${last_day}')
select
    report_type,
    seller_id,
    a.marketplace_id,
    ...
    data_dt ,
    etl_data_dt
from temp3_dwd_trd_ord_amazon_flat_file_all_orders_report_dk a
left outer join (select * from dim.dim_base_marketplace_info_df where ds ='${last_day}')  b
