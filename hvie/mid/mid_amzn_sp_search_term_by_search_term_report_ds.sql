--drop table if exists amz.mid_amzn_sp_search_term_by_search_term_report_ds;
CREATE TABLE IF NOT EXISTS amz.mid_amzn_sp_search_term_by_search_term_report_ds(
...
WITH dwd_adv_sp_search_term_by_search_term_report_df_temp as (
    SELECT
 ...
 (
  SELECT
   ...
   ,ROW_NUMBER() OVER (PARTITION BY tenant_id,profile_id,campaign_id,ad_group_id,keyword_id,search_term,report_date ORDER BY ds DESC ) AS rn
   ,ds
  FROM ods.ods_report_amzn_sp_search_term_by_search_term_report_df
  WHERE ds ='${last_day}'
) s
WHERE rn=1
    ),n_lag_table AS (
    SELECT
        CASE
            ...
            ELSE ${n_lag2}
            END AS n_lag
    FROM dwd_adv_sp_search_term_by_search_term_report_df_temp
    WHERE ds = '${last_day}'
)

-- Step 2: 使用计算的 n_lag 插入数据到目标表
INSERT OVERWRITE TABLE amz.mid_amzn_sp_search_term_by_search_term_report_ds PARTITION (ds)
SELECT
    ...
    ad_group_name, keyword_id, keyword, keyword_type, keyword_bid, match_type, targeting, ad_keyword_status, search_term, impressions, clicks,
    cost_per_click, click_through_rate, cost, purchases_1d, purchases_7d, purchases_14d, purchases_30d, purchases_same_sku_1d, purchases_same_sku_7d,
    ...
    acos_clicks_7d, acos_clicks_14d, roas_clicks_7d, roas_clicks_14d,data_src,table_src,
...
FROM (
         SELECT
             ...
             acos_clicks_7d, acos_clicks_14d, roas_clicks_7d, roas_clicks_14d,data_src,table_src,data_dt,
             ROW_NUMBER() OVER (PARTITION BY tenant_id, profile_id, campaign_id, ad_group_id, keyword_id, search_term, report_date ORDER BY current_date() DESC) AS rn
         FROM dwd_adv_sp_search_term_by_search_term_report_df_temp
