--  取代 adm_amazon_adv_strategy_pasin_search_term_d
CREATE TABLE IF NOT EXISTS amz.dws_stg_pasin_search_term_d(
    ...
    camp_name_list STRING COMMENT '包含的广告活动名称列表(通过_&_拼接)',
    camp_id_list STRING COMMENT '包含的广告活动ID列表(通过_&_拼接)',
    ad_group_id_list STRING COMMENT '包含的广告组id列表(通过_&_拼接)',
    ...
    STORED AS ORC
    TBLPROPERTIES ('comment'='亚马逊广告策略搜索词数据(父asin+搜索词粒度，不含广告活动，方便各租户使用，统计最近30天)')
    ;

INSERT OVERWRITE TABLE amz.dws_stg_pasin_search_term_d PARTITION (ds = '${bizdate}')
SELECT  a.tenant_id
     ,profile_id
     ,a.marketplace_id
     ,marketplace_name
...
FROM    (
            SELECT  s1.tenant_id
                 ...
                 ,match_type
                 ,search_term
--                  ,WM_CONCAT(DISTINCT '_&_',s3.campaign_name) camp_name_list
--                  ,WM_CONCAT(DISTINCT '_&_',s1.campaign_id) camp_id_list
--                  ,WM_CONCAT(DISTINCT '_&_',CONCAT(s1.campaign_id,'_/_',s1.ad_group_id)) ad_group_id_list
-- 组合所有 campaign_name
                 ,CONCAT_WS(',', COLLECT_SET(CONCAT('_&_', s3.campaign_name))) AS camp_name_list

-- 组合所有 campaign_id
                ,CONCAT_WS(',', COLLECT_SET(CONCAT('_&_', s1.campaign_id))) AS camp_id_list,

-- 组合所有 ad_group_id
                ,CONCAT_WS(',', COLLECT_SET(CONCAT('_&_', s1.campaign_id, '_/_', s1.ad_group_id))) AS ad_group_id_list
    ...
    WHERE   ws = '${bizdate}'-- MAX_PT('dwd_itm_spu_amazon_search_keyword_info_ws')
    ...
    FROM amz.dwd_prd_parent_asin_index_df   -- whde.dws_itm_sku_amazon_asin_index_df
    WHERE   ds ='${bizdate}' -- MAX_PT('whde.dws_itm_sku_amazon_asin_index_df')
      AND     NVL(parent_asin,'') <> ''
    GROUP BY marketplace_id
           ,parent_asin
) f
