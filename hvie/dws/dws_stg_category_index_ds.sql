...
CREATE TABLE IF NOT EXISTS amz.dws_stg_category_index_ds
(
...
--类目平均基础数据准备
drop table if exists dws_mkt_adv_strategy_category_index_tmp0;
create table dws_mkt_adv_strategy_category_index_tmp0 as
select   q1.tenant_id
     ,q1.profile_id
     ,q1.marketplace_id
     ,q1.marketplace_name
     ...
     ,'' as category
     ,'' as self_category
     ,'' as fba_first_instock_days
     ,'' as fba_first_instock_from_days
...
--      ,datediff(
--         from_unixtime(unix_timestamp(q1.ds, 'yyyymmdd'), 'yyyy-MM-dd'),
--         substr(q2.fba_first_instock_time, 1, 10)
--       ) as fba_first_instock_from_days
     ,q1.ds
from    (
            select   s1.tenant_id
...
--                         where   ds between to_char(dateadd(to_date('${bizdate}','yyyymmdd'),-90,'dd'),'yyyymmdd') and '${bizdate}'
                              where   ds between date_format(date_sub(to_date(from_unixtime(unix_timestamp('20240827', 'yyyyMMdd'))), 90),'yyyyMMdd')  and '${bizdate}'
                ...
                where   ds ='${bizdate}' --  max_pt('whde.adm_amazon_adv_sku_wide_d')
                group by  tenant_id
                       ,profile_id
                       ,marketplace_id
                       ,marketplace_name
...
from  dws_mkt_adv_strategy_category_index_tmp0
...
select  tenant_id
     ...
     ,case when sum(sale_amt) = 0 then null else sum(cost)/sum(sale_amt) end as cate_acos
from  dws_mkt_adv_strategy_category_index_tmp0
where ds >= date_format(date_sub(to_date(from_unixtime(unix_timestamp('20240827', 'yyyyMMdd'))), 90),'yyyyMMdd' )  -- to_char(dateadd(to_date('${bizdate}','yyyymmdd'), -90, 'dd'),'yyyymmdd')
--and nvl(self_category,'') <> ''
group by tenant_id
       ,marketplace_id
       ,marketplace_name
...
--支持重跑
alter table amz.dws_stg_category_index_ds drop if exists partition (ds = '${bizdate}');

--插入最新数据
insert into table amz.dws_stg_category_index_ds partition (ds = '${bizdate}')
(
 tenant_id
,marketplace_id
,marketplace_name
