...
--********************************************************************--
CREATE TABLE IF NOT EXISTS amz.dws_adv_camp_budget_lack_rate_ds
(
...
with dws_mkt_adv_camp_budget_lack_rate_ds_temp2 as (
select   tenant_id
     ,profile_id
     ,marketplace_id
     ,seller_id
...
--               ,to_date(report_date,'yyyy-mm-dd') as report_date
              ,to_date(date_format(report_date, 'yyyy-MM-dd')) as report_date
              ,update_time_row
              ,update_time_zh
              ,sum(cost) cost
              ,max(campaign_budget_amount) as campaign_budget_amount
         from  amz.dwd_adv_sp_product_ms   -- whde.dwd_mkt_adv_amazon_sp_product_ms
         where replace(report_date,'-','')  = substr(update_time_row,1,8)
           and substr(ms,1,8)<='20240827'
           and substr(ms,1,8)>='20240827'
         group by  tenant_id
                ,profile_id
                ,marketplace_id
                ,seller_id
                ,campaign_id
                ,to_date(date_format(report_date, 'yyyy-MM-dd'))
                ,update_time_row
                ,update_time_zh
     ) q1
...
,dws_mkt_adv_camp_budget_lack_rate_ds_temp3 as (
select  tenant_id
     ,profile_id
     ,seller_id
     ,campaign_id
...
from (
         ...
         where report_date < '20240828' and substr(update_time_row,9,2) >= '07' and substr(update_time_row,9,2) <= '22'
...
-- create table
   ,dws_mkt_adv_camp_budget_lack_rate_ds_temp4 as (
select  tenant_id
     ,profile_id
     ,seller_id
     ,campaign_id
...
from (
         select   tenant_id
              ...
              ,sum(is_lack_budget) / count(1) as budget_rate
         from  dws_mkt_adv_camp_budget_lack_rate_ds_temp2
         where report_date > date_sub(to_date(from_unixtime(unix_timestamp('20240827', 'yyyyMMdd'))), 7)  and report_date < '2024-08-27' and substr(update_time_row,9,2) >= '07' and substr(update_time_row,9,2) <= '22'
...
,dws_mkt_adv_camp_budget_lack_rate_ds_temp5 as (
select  tenant_id
     ,profile_id
     ,update_time_zh
     ,campaign_id
...
-- create table dws_mkt_adv_camp_budget_lack_rate_ds_temp6 as
   , dws_mkt_adv_camp_budget_lack_rate_ds_temp6 as (
select  tenant_id
     ,profile_id
     ,campaign_id
...
from (
         select   tenant_id
              ...
              ,max(cost) as cost
         from  dws_mkt_adv_camp_budget_lack_rate_ds_temp2
         where report_date > date_sub(to_date(from_unixtime(unix_timestamp('20240827', 'yyyyMMdd'))), 7)  and report_date <  '2024-08-27'
...
-- create table
    ,dws_mkt_adv_camp_budget_lack_rate_ds_temp7 as (
select   tenant_id
     ,profile_id
     ,campaign_id
     ,max(campaign_budget_amount) as campaign_budget_amount
     ,sum(is_lack_budget) / count(1) as budget_rate
from  dws_mkt_adv_camp_budget_lack_rate_ds_temp2
where report_date > date_sub(to_date(from_unixtime(unix_timestamp('20240827', 'yyyyMMdd'))), 1)  and report_date <  '2024-08-27' and substr(update_time_row,9,2) >= '07' and substr(update_time_row,9,2) <= '22'
...
-- create table
  ,  dws_mkt_adv_camp_budget_lack_rate_ds_temp8 as (
select   q1.tenant_id
     ,q1.profile_id
     ,q1.marketplace_id
     ,q1.seller_id
...
insert OVERWRITE  table amz.dws_adv_camp_budget_lack_rate_ds partition (ds = '20240827')
select tenant_id
     ,profile_id
     ,marketplace_id
     ,seller_id
